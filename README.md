...
for Windows, Linux, macOS (→ [Download](https://github.com/Lesserkuma/FlashGBX/#Downloads))
...
- Portable: Extract to a folder of your choice and run FlashGBX without having to install anything.
- Setup: Installable package that adds the application to the start menu and optionally creates a desktop icon, also includes device drivers.
...
* To upgrade to the latest version, use the following commands:<br>`source FlashGBX_venv/bin/activate`<br>`python3 -m pip install -U "FlashGBX"`
